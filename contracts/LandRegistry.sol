...
contract LandRegistry {
    ...
    mapping(bytes32 => bool) public uniqueIdHashExists; // Prevents duplicate ID submissions
    mapping(bytes32 => bool) public uniqueDeedIdHashExists; // Prevents duplicate deed submissions
     ...
     */
    function submitVerification(string memory _documentIpfsHash, bytes32 _uniqueIdHash) public {
        require(bytes(_documentIpfsHash).length > 0, "Document IPFS hash cannot be empty");
        require(_uniqueIdHash != bytes32(0), "Unique ID hash cannot be empty");
        require(!uniqueIdHashExists[_uniqueIdHash], "This unique ID has already been submitted");
        ...
        uniqueIdHashExists[_uniqueIdHash] = true;
    ...
    // --- Land Registration Functions ---

    /**
     * @dev Initiates a new land registration request with a unique deed hash.
     */
    function initiateRegistration(string memory _location, string memory _ipfsHash, bytes32 _uniqueDeedIdHash) public onlyVerifiedUser {
        ...
        require(_uniqueDeedIdHash != bytes32(0), "Unique deed ID hash cannot be empty");
        require(!uniqueDeedIdHashExists[_uniqueDeedIdHash], "This deed has already been submitted for registration");
        ...
        uniqueDeedIdHashExists[_uniqueDeedIdHash] = true; // Mark deed as submitted
    ...
    /**
     * @dev Allows an authority to reject a registration request with a reason.
     */
    function rejectRegistration(uint _registrationId, string memory _reason) public onlyAuthority {
        Registration storage r = registrations[_registrationId];
        require(_registrationId > 0 && _registrationId <= registrationCounter, "Invalid registration ID");
        require(!r.executed, "Registration already executed");
        require(r.approvals[msg.sender] == ApprovalStatus.PENDING, "Already voted on this registration");
        ...
        if (!r.isRejected) {
            ...
            r.rejectionReason = _reason; // Save the reason
        }
    ...
    function resubmitRegistration(uint _registrationId, string memory _newLocation, string memory _newIpfsHash) public {
        ...
        r.rejectionReason = ""; // Clear the old reason
        r.approvals[KITWE_CITY_COUNCIL] = ApprovalStatus.PENDING;
        r.approvals[COMMISSIONER_OF_LAND] = ApprovalStatus.PENDING;
        r.approvals[LANDS_DEEDS_REGISTRY] = ApprovalStatus.PENDING;
        r.approvals[TREASURY] = ApprovalStatus.PENDING;
